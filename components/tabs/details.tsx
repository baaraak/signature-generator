...
import { AiOutlineCamera, AiOutlineDelete, AiOutlineEdit, AiOutlineMore, AiOutlinePlus, AiOutlineUnorderedList } from 'react-icons/ai';
    ...
    <div className="p-6 transition animate-enter-left">
      ...
      <div className="flex">
        <div className="flex-1">
          {details.map(row => (
            <InputWithActions key={row.id} className="mb-7" onBlur={onChange} {...row} />
          ))}
...
const InputWithActions = ({mandatory, ...inputProps}: any) => {
  ...
  return (
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} className="relative flex -ml-4">
      <AiOutlineMore className={`mr-1 mt-2 ${hovered ? 'opacity-1' : 'opacity-0'} transition duration-500 cursor-move`} />
    <Input {...inputProps} className="mb-7 flex-1"/>
      {!mandatory && (
        <>
          <AiOutlineDelete className={`absolute right-6 top-1 text-xl text-red-400  ${hovered ? 'opacity-1' : 'opacity-0'} transition duration-500 cursor-pointer`} />
          <AiOutlineEdit className={`absolute right-0 top-1 text-xl text-blue-400  ${hovered ? 'opacity-1' : 'opacity-0'} transition duration-500 cursor-pointer`} />
        </>
      )}
  </div>
  )
